...
def test_set_volume1(setup):
    ...
    channel = 0
    #            channel, tick,start,level, delta, rate
    mv.set_level(channel,    0, None,  100,  None,    0)
    assert vlist(-1).level == 100
    mv.set_level(channel,  500, None,   80,  None,    0)
    assert vlist(-1).level == 80
    mv.set_level(channel,  800, None,   60,  None,    0)
...
def test_set_volume2(setup):
    ...
    delta = 40
    rate = 0
    mv.set_level(channel, tick, None, level, delta, rate)
...
def test_set_volume3(setup):
    ...
    delta = 20
    rate = 2
    mv.set_level(channel, tick, None, None, delta, rate)
    prev = vlist(-2)
    last = vlist(-1)
...
def test_set_volume4(setup):
    ...
    #            channel, tick,start,level, delta, rate
    mv.set_level(channel,    0, None,  100,  None,    0)
    mv.set_level(channel, 1000, None,   40,  None,   20)
    prev = vlist(-2)
    last = vlist(-1)
...
def test_set_volume4neg(setup):
    ...
    delta = None
    rate = 0
    mv.set_level(channel, tick, None, level1, delta, rate)
    assert vlist(-1).level == level1
    tick = 500
    level2 = 40
    delta = None
    rate = 2
    mv.set_level(channel, tick, None, level2, delta, rate)
    prev = vlist(-2)
    last = vlist(-1)
    assert prev.tick == tick
...
def test_set_volume1_in_change(setup):
    ...
    #            channel, tick,start,level, delta, rate
    mv.set_level(channel,    0, None,  100,  None,    0)
    mv.set_level(channel, 1000, None,   80,  None,    0)
    mv.set_level(channel, 3000, None, None,   -30,   10)
    mv.set_level(channel, 4000, None,   90,  None,    0)
    last = vlist(-1)
    assert last.tick == 4000
...
def test_set_volume4_in_change(setup):
    ...
    mv.set_level(channel,    0, None,  100,  None,    0)
    mv.set_level(channel, 1000, None,   80,  None,    0)
    mv.set_level(channel, 3000, None, None,   -30,   10) # creates(6000,50,10)
    mv.set_level(channel, 4000, None,   90,  None,   10)
    prev = vlist(-2)
    last = vlist(-1)
...
def test_get_volume2(setup):
    """Test case 4s, i.e. start supplied."""
    channel = 0
    #            channel, tick,start,level, delta, rate
    mv.set_level(channel,    0, None,  100,  None,    0)
    mv.set_level(channel, 1000, None,   80,  None,    0)
    mv.set_level(channel, 3000,   60,   30,  None,   10)
    assert mv.get_level(channel, 0) == 100
    assert mv.get_level(channel, 999) == 100
    assert mv.get_level(channel, 1000) == 80
    assert mv.get_level(channel, 2999) == 80
    assert mv.get_level(channel, 3000) == 60    # supplied by start
    assert mv.get_level(channel, 4000) == 50
    assert mv.get_level(channel, 5000) == 40
    assert mv.get_level(channel, 6000) == 30    # reached target level
    assert mv.get_level(channel, 7000) == 30
